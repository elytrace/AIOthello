...
public class AIGamePanel extends JPanel implements Runnable, MouseListener {
    ...
    private void update() {
        ...
        int location;
        if(player1Turn) location = x + y * 8;
        else {
            location = Minimax.best_solution(position);

            // Random ran = new Random();
            // location = ran.nextInt(64);
            // for(int i = 0; i < possible_moves.size(); i++) {
            //     System.out.print(possible_moves.get(i) + " ");
            // }
            // System.out.println("\n" + location);
            try {
                Thread.sleep(500);
        ...
        if(location < 0 || location > 63) return;
        ...
        playerScore[1] = 0;
        for(int i = 0; i < 64; i++) {
            if(position[i] == 1) playerScore[0]++;
            else if(position[i] == 2) playerScore[1]++;
        }
    }
    ...
    private void render() {
        ...
        for(int i = 0; i < 8; i++) {
            for(int j = 0; j < 8; j++) {
                ...
                // g.drawString(String.valueOf(i * 8 + j), j * 60 + 43, i * 60 + 177);
            }
        }
        if(player1Turn)
            for(int i = 0; i < possible_moves.size(); i++) {
